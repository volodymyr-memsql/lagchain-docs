...
`ChatOpenRouter` supports structured output via the [`with_structured_output`](/oss/langchain/models#structured-output) method. Two methods are available: `function_calling` (default) and `json_schema`.
...
For more on content blocks, see the [standard content blocks](/oss/langchain/messages#standard-content-blocks) guide.
