...
| **Per-provider/model tuning** | [Harness profiles](/oss/deepagents/profiles) (beta): declarative bundles of system prompt, tool, middleware, and subagent tweaks, registered per provider or specific model | Configure in code at each model call site |
...
<Tip>
    [Claude managed agents](https://platform.claude.com/docs/en/managed-agents/overview) work well within the Anthropic ecosystem; for a similar low-friction, model-agnostic path in LangChain, use [LangSmith Fleet](/langsmith/fleet/index).
</Tip>
...
The Claude Agent SDK is purpose-built for Claude and Anthropic’s product surface. If you are already standardizing on the Anthropic ecosystem (Claude API and related platform features), staying on the official SDK can mean fewer moving parts, a single vendor for agent features and support, and documentation that assumes the same stack end to end.
