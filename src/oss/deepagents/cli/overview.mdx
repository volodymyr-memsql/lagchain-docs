...
CLI flags are session-only. `--model-params` cannot be combined with `--default`. For persistent provider-level defaults, per-model overrides, and provider-specific examples, see [Model parameters](/oss/deepagents/cli/providers#model-parameters) on the providers page.
...
The frontmatter requires `name` and `description` (same as the [`SubAgent` dictionary spec](/oss/deepagents/subagents#subagent-dictionary-based)). The markdown body becomes the subagent's `system_prompt`. In addition to the base spec, `AGENTS.md` files support an optional `model` frontmatter field that overrides the main agent's model for this subagent. Uses the `provider:model-name` format (e.g., `anthropic:claude-opus-4-6`, `openai:gpt-5.5`). Omit to inherit the main agent's model.
