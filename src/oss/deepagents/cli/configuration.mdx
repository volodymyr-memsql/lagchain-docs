...
<Tip>
    For one-off adjustments without editing `config.toml`, pass a JSON object via `--model-params` at launch or mid-session with `/model`. CLI flags take highest priority over the config file. See [Model parameters](/oss/deepagents/cli/providers#model-parameters) on the providers page for syntax and provider-specific examples.
</Tip>
...
By default, when the CLI loads skills it validates that a resolved skill file path stays inside one of the standard [skill directories](/oss/deepagents/data-locations#skills). This prevents symlinks inside skill directories from reading arbitrary files outside those roots.
...
<ResponseField name="DEEPAGENTS_CLI_LANGSMITH_PROJECT" type="string" post={["optional"]}>
    Override the LangSmith project name for agent traces. See [Tracing with LangSmith](/oss/deepagents/cli/overview#tracing-with-langsmith).
</ResponseField>
