...
Specify models in `provider:model` format (for example, `anthropic:claude-sonnet-4-6` or `openai:gpt-5.4`). For valid provider strings, see the `model_provider` parameter of @[`init_chat_model`]. For provider-specific configuration, see [chat model integrations](/oss/integrations/chat).
