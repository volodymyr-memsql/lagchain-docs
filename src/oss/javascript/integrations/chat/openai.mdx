...
This guide will help you getting started with OpenAI [chat models](/oss/langchain/models). For detailed documentation of all `ChatOpenAI` features and configurations head to the [API reference](https://reference.langchain.com/javascript/langchain-openai/ChatOpenAI).
...
Equipping @[`ChatOpenAI`] with built-in tools will ground its responses with outside information, such as via context in files or the web. The [AIMessage](/oss/langchain/messages#ai-message) generated from the model will include information about the built-in tool invocation.
