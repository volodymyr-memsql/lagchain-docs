...
For a full list of endpoints and request/response schemas, refer to the [API reference](https://api.smith.langchain.com/redoc).
...
def post_run(run_id, name, run_type, inputs, parent_id=None):
    ...
    requests.post(
        "https://api.smith.langchain.com/runs",  # Update for self-hosted, EU (GCP) (`eu.api...`), or US (AWS) (`aws.api...`)
        json=data,
        headers=headers
    )
...
api_url = "https://api.smith.langchain.com"  # EU (GCP): eu.api...; US (AWS): aws.api... for regional SaaS
