...
Pass [`run_type="retriever"`](/langsmith/run-data-format) to the @[traceable] decorator (Python) or `traceable` wrapper (TypeScript). This tells LangSmith to treat the step as a retrieval run and apply retriever-specific rendering in the LangSmith UI.
