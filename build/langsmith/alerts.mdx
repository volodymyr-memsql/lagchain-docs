...
<Tabs>
  ...
  <Tab title="Dynatrace">
    ...
    **Prerequisites**
    ...
    1. Click the key icon to save the API token as a workspace secret or select an existing workspace secret. As a best practice, save the API token as a workspace secret rather than adding it directly. This allows you to re-use the same token across alerts for a workspace.
