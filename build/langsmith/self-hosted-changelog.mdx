...
- Sandbox list APIs now support filtering by status/template, sorting by any column, and searching by UUID or image.
...
- Sandbox tables now support filtering by status, sorting by any column, searching by UUID/image, and inline edit/delete actions.
- Sandbox batch delete API endpoints now allow deleting multiple boxes or templates in a single request.
- Sandbox tables now support multi-select batch delete with feedback on skipped items.
- Sandbox detail page now has "Open" and "Share Link" buttons to access HTTP services running inside sandboxes directly from the browser.
- Fixed crash when scrolling quickly through sandbox tables.
...
- Added sandbox service URL API to generate short-lived tokens for accessing HTTP services running inside sandboxes from the browser or programmatically.
...
- Added a unified files sidebar to Agent Builder chat, accessible via a "Files" button in the header, to browse, search, create, rename, move, and preview all agent-generated files in one panel.
...
- Removed the legacy Feed page in Agent Builder; Inbox is now the default thread view for all tenants.
...
- Added Salesforce SOQL query tool to Agent Builder, enabling agents to query Salesforce data via OAuth.
...
- Added typed proxy headers for sandbox boxes with opaque (write-only encrypted) and workspace secret reference types.
...
- Fixed TLS certificate verification failures for Node.js, npm, pip, and Codex CLI when using the sandbox proxy.
