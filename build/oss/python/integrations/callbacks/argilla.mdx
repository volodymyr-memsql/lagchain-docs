...
Finally, as a more advanced workflow, you can create an agent that uses some tools. The `ArgillaCallbackHandler` will keep track of the input and the output, but not the intermediate steps/thoughts. Given a prompt, we log the original prompt and the final response.
