...
If you do not have a Trubrics account, [create one on Trubrics](https://trubrics.streamlit.app/). In this tutorial, we will use the `default` project that is built upon account creation.
...
The `TrubricsCallbackHandler` can receive various optional arguments. See [Trubrics prompt kwargs documentation](https://trubrics.github.io/trubrics-sdk/platform/user_prompts/#saving-prompts-to-trubrics) for kwargs that can be passed to Trubrics prompts.
