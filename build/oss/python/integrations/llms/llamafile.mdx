...
Now you can make calls to the llamafile's REST API. By default, the llamafile server listens at [localhost:8080](http://localhost:8080). You can find full server documentation in the [llama.cpp server API endpoints README](https://github.com/Mozilla-Ocho/llamafile/blob/main/llama.cpp/server/README.md#api-endpoints). You can interact with the llamafile directly via the REST API, but here we'll show how to interact with it using LangChain.
