...
`GraphDB` supports some LLM integration functionalities as described [in the W3C SPARQL dev discussion](https://github.com/w3c/sparql-dev/issues/193):
...
For this tutorial, we will not use the GraphDB LLM integration, but `SPARQL` generation from NLQ. We'll use the `Star Wars API` (`SWAPI`) ontology and dataset that you can examine [in the demo repository](https://github.com/Ontotext-AD/langchain-graphdb-qa-chain-demo/blob/main/starwars-data.trig).
