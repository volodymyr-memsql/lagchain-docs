...
Note: When creating a new instance of TimescaleVector, the TimescaleVector Module will try to create a table with the name of the collection. Make sure that the collection name is unique (i.e it doesn't already exist).
...
Important note: In PostgreSQL, each table can only have one index on a particular column. If you'd like to test the performance of different index types, you can do so either by (1) creating multiple tables with different indexes, (2) creating multiple vector columns in the same table and creating different indexes on each column, or (3) by dropping and recreating the index on the same column and comparing results.
